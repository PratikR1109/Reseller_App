...
module.exports = {
	...
	Routes: (router) => {
					...
					db.collection('destination_code').find({
						...
						db.collection('orderproducts').find({
							...
							db.collection('manifestfiles').find({
								...
								console.log("address :::::::: ", senderName[0]);
								...
								pdf.create(html, options).toStream(function (err, stream) {
									console.log(results[0].senderDetails + ' ' + address + ' ')
		...
		router.post('/vendorlist/:page', (req, res) => {
			...
			if (typeof sess.user != 'undefined') {
				...
				db.collection('vendor').count(wh, (err, totalCount) => {
					db.collection('vendor').find(wh).skip(skip).limit(perPage).sort({ created_at: -1 }).toArray((err, results) => {
						if (results.length) {
							for (var i = 0; i < results.length; i++) {
								results[i].created_at = _date(results[i].created_at).format('DD MMMM, YYYY HH:mm:ss');
							}
					...
					db.collection('products').aggregate(aggregated).toArray((err, results) => {
						if (results.length) {
							...
							for (var i = 0; i < results.length; i++) {
								...
								db.collection('orderproducts').count({
									productId: results[i].productId
								}, (err, orderCount) => {
									// console.log('Order : '+orderCount)
									data['totalOrder'] = orderCount;
								})
							}
							data['data'] = results;
							data['search'] = {
		...
		router.post('/productlist/:page', (req, res) => {
			...
			console.log('page No :: '+req.params.page)
			var page = (typeof req.params.page != 'undefined') ? (req.params.page == 0) ? 1 : req.params.page || 1 : 1;
			var skip = (perPage * page) - perPage;
			var limit = "LIMIT " + skip + ", " + perPage;
								...
								db.collection('catalogs').find({
									...
									db.collection('products').find({
										...
										if (results.length) {
											...
											console.log("Insert Data :: ",productDetails);
		...
		// 												db.collection('products').insertOne(productDetails, (err, results) => {
		// 													if (!err) {
		// 														return webHandler.responseData('addproduct.html', data, res);
		// 														// return res.redirect('/productlist/1');
		// 													} else {
		// 														sess = req.session;
		// 														sess.error = 'Data Not Saved!(data base Error)';
					...
					db.collection('addresses').find({
						...
						if (results.length) {
							...
							if (typeof results[0]['phone_no'] != 'undefined') {
								...
								orderplace.mobileno = results[0]['phone_no']
							}
							db.collection('sender').find({
		...
		router.post('/orderlist/:page', (req, res) => {
			...
			if (typeof sess.user != 'undefined') {
				...
				if (typeof req.body.orderno != 'undefined' && req.body.orderno != '') {
					if (req.body.byList != '') {
						if (req.body.byList == 'Orderno') {
							wh['orderno'] = parseInt(req.body.orderno);
						} else if (req.body.byList == 'Mobileno') {
							wh['mobileno'] = req.body.orderno;
						} else if (req.body.byList == 'ResellerID') {
							wh['resellerId'] = req.body.orderno;
						}
					}
					...
					if (typeof req.file != 'undefined' && typeof req.file.filename != 'undefined') {
						db.collection('catalogs').find({
							...
							if (results.length) {
								...
								db.collection('catalogs').find({ }).sort({ _id : -1 }).toArray((err, lastCatalog) => {
						...
						if (catalogresults.length) {
							...
							db.collection('catalogs').aggregate(aggregated).toArray((err, results) => {
								...
								temp(data['data'], 0, (cb) => {
									data['data'] = cb;
									c("data ::::::::::: cataloglist list ", data);
									console.log("data ::::::::::: cataloglist list ", data);
									webHandler.responseData('cataloglist.html', data, res);
								})
								...
								function temp(data, i, done) {
									if (data.length) {
										data[i].vendorId = typeof data[i].vendorId != 'undefined' ? data[i].vendorId.length == 24 ? data[i].vendorId : '000000000000000000000000' : '000000000000000000000000';
										console.log('vendor ID : '+data[i].vendorId+' :: and i : '+i);
										db.collection('vendor').find({
											_id: new MongoId(data[i].vendorId)
										}).toArray((err, results) => {
											if (results.length) {
											...
											console.log('vendor Name : '+data[i].vendorName+' :: and i : '+i);
	...
	updateProductDetails: (data, file, done) => {
		c("file ::::: ", file);
		console.log("Data Data 1.o:: ",data);
		if (file != '') {
			var productData = {};
		...
		if (typeof data.discount_pre != 'undefined' && data.discount_pre != '') {
			productData.discount_pre = parseInt(data.discount_pre);
			productData.discount_price = productData.price - ((productData.discount_pre * productData.price) / 100);
			console.log("UPdate Data 1.o:: ",productData.discount_price);
		}
		else{	
			productData.discount_pre=0;
			productData.discount_price = parseInt(data.price);
		}
		...
		if (!_.isEmpty(productData)) {
			db.collection('products').update({
				...
				if(productData.discount_pre == 0){
					db.collection('products').update({ _id: new MongoId(data.productId) }, { $unset : {discount_price : 1 } }, (e, r) => { 
						if(!e){
